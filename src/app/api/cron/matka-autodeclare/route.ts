...
function pattiAnk(p: string) { return p.split('').reduce((s, d) => s + parseInt(d), 0) % 10; }
...
async function safestPatti(marketId: string, session: 'OPEN' | 'CLOSE', m: any): Promise<string> {
  const bets = await prisma.matkaBet.findMany({ where: { marketId, status: 'ACTIVE' }, select: { betType: true, betValue: true, session: true, amount: true } });
  ...
  let bestPatti = '100'; let bestPayout = Infinity;
  for (const patti of candidates) {
    const ank = pattiAnk(patti); let payout = 0;
    for (const bet of bets) {
      const bt = (bet.betType ?? '').toUpperCase(); const bv = bet.betValue ?? '';
...
export async function GET() {
  try {
    ...
    for (const m of markets) {
      const mkt = m as any;
      if (mkt.saleDatetime && !m.isOpen && !m.isResultDeclared && now >= new Date(mkt.saleDatetime)) {
        await prisma.matkaMarket.update({ where: { id: m.id }, data: { isOpen: true } });
        log.push(`OPENED: ${m.name}`);
      }
      if (mkt.openDatetime && m.isOpen && now >= new Date(mkt.openDatetime)) {
        const today = new Date(); today.setHours(0, 0, 0, 0);
        const existing = await prisma.matkaResult.findFirst({ where: { marketId: m.id, createdAt: { gte: today } }, orderBy: { createdAt: 'desc' } });
        if (!existing?.openPatti) {
          const patti = await safestPatti(m.id, 'OPEN', m); const ank = pattiAnk(patti);
          const result: any = !existing
            ? await prisma.matkaResult.create({ data: { marketId: m.id, openPatti: patti, openAnk: ank, declaredAt: now } })
            : await prisma.matkaResult.update({ where: { id: existing.id }, data: { openPatti: patti, openAnk: ank, declaredAt: now } });
          const RATES: Record<string, number> = { ANK: m.payoutSingle, SINGLE_ANK: m.payoutSingle, SP: m.payoutSP, SINGLE_PATTI: m.payoutSP, DP: m.payoutDP, DOUBLE_PATTI: m.payoutDP, TP: m.payoutTP, TRIPLE_PATTI: m.payoutTP };
      ...
      if (mkt.closeDatetime && m.isOpen && !m.isResultDeclared && now >= new Date(mkt.closeDatetime)) {
        const today = new Date(); today.setHours(0, 0, 0, 0);
        const result = await prisma.matkaResult.findFirst({ where: { marketId: m.id, createdAt: { gte: today } }, orderBy: { createdAt: 'desc' } });
        if (result?.openPatti && !result.closePatti) {
          const closePatti = await safestPatti(m.id, 'CLOSE', m); const closeAnk = pattiAnk(closePatti);
          const openAnk = result.openAnk ?? pattiAnk(result.openPatti); const jodi = `${openAnk}${closeAnk}`;
          await prisma.matkaResult.update({ where: { id: result.id }, data: { closePatti, closeAnk, jodi, declaredAt: now } });
          const RATES: Record<string, number> = { ANK: m.payoutSingle, SINGLE_ANK: m.payoutSingle, JODI: m.payoutJodi, SP: m.payoutSP, SINGLE_PATTI: m.payoutSP, DP: m.payoutDP, DOUBLE_PATTI: m.payoutDP, TP: m.payoutTP, TRIPLE_PATTI: m.payoutTP, HALF_SANGAM: m.payoutHalfSangam, FULL_SANGAM: m.payoutFullSangam };
          ...
          await prisma.matkaMarket.update({ where: { id: m.id }, data: { isOpen: false, isResultDeclared: true } });
